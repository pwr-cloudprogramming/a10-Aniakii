...
function inputCredentials() {
  if (
    localStorage.getItem("aws-congnito-user-pool-id") !== undefined &&
    localStorage.getItem("aws-congnito-app-id") !== undefined
  ) {
    document.getElementById("cognitoUserPoolId").value = localStorage.getItem(
      "aws-congnito-user-pool-id",
    );
    document.getElementById("applicationId").value = localStorage.getItem(
      "aws-congnito-app-id",
    );
  }
